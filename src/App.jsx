...
function ProjectView({ project, expenses, globalMembers, onBack }) {
  ...
  const projectMembers = useMemo(() => globalMembers.filter(m => project.memberIds?.includes(m.id)), [globalMembers, project.memberIds]);
...
function TabButton({ icon, label, active, onClick, className = '' }) {
  return (
    <button onClick={onClick} className={`flex flex-col items-center justify-center p-2 w-16 gap-1 transition-colors ${active ? 'text-[#52B4CC] font-bold' : 'text-gray-400 hover:text-[#8BCDDD]'} ${className}`}>
      {React.cloneElement(icon, { size: active ? 24 : 22 })}<span className="text-[10px]">{label}</span>
    </button>
  );
}
...
function ProjectSettings({ project, globalMembers, onBack }) {
  ...
  const handleToggleDecimals = async () => { try { await updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'projects', project.id), { showDecimals: !showDecimals }); setShowDecimals(!showDecimals); } catch(e) {} };
  const handleToggleSettled = async () => { try { await updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'projects', project.id), { isSettled: !project.isSettled }); } catch(e) {} };
  const handleDeleteProject = async () => { try { await deleteDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'projects', project.id)); onBack(); } catch(e) {} };
...
function ExpenseForm({ project, members, initialData, onSuccess, onCancel }) {
  ...
  useEffect(() => {
    if (initialData) {
      setDate(initialData.date); setTitle(initialData.title); setNote(initialData.note || ''); setPayerId(initialData.payerId); setCurrency(initialData.currency); setTotalAmount(initialData.totalAmount.toString()); setSplitMode(initialData.splitMode);
      if (initialData.splitMode === 'equal') setSelectedSplitters(Object.keys(initialData.splits)); else setUnequalSplits(initialData.splits);
    } else { setSelectedSplitters(members.map(m => m.id)); }
  }, [initialData, members]);
  ...
  const handleToggleSplitter = (id) => { if (selectedSplitters.includes(id)) setSelectedSplitters(selectedSplitters.filter(s => s !== id)); else setSelectedSplitters([...selectedSplitters, id]); };

  const handleSave = async () => {
    if (!title.trim() || !payerId || !totalAmount || Number(totalAmount) <= 0) return;
    let splits = {}; let isPersonal = false;
    if (splitMode === 'equal') {
    ...
    const payload = { projectId: project.id, date, title: title.trim(), note: note.trim(), payerId, currency, totalAmount: Number(totalAmount), splitMode, splits, isPersonal, updatedAt: Date.now() };
    try {
    ...
    <div className="p-4 space-y-4 animate-in fade-in pb-24">
      <div className="bg-white p-4 rounded-xl shadow-sm space-y-4 border border-gray-100">
        <div className="flex gap-3">
          <div className="w-1/3"><label className="text-xs font-bold text-gray-500 block mb-1">日期</label><input type="date" value={date} onChange={e=>setDate(e.target.value)} className="w-full border border-gray-200 p-2 rounded-lg bg-gray-50 text-sm outline-none" /></div>
          <div className="flex-1"><label className="text-xs font-bold text-gray-500 block mb-1">品項名稱</label><input type="text" placeholder="例如：晚餐、計程車" value={title} onChange={e=>setTitle(e.target.value)} className="w-full border border-gray-200 p-2 rounded-lg bg-gray-50 outline-none" /></div>
        </div>
        <div><label className="text-xs font-bold text-gray-500 flex items-center gap-1 mb-1"><StickyNote size={12}/> 備註 (選填)</label><input type="text" placeholder="有什麼想記下來的嗎？" value={note} onChange={e=>setNote(e.target.value)} className="w-full border border-gray-200 p-2 rounded-lg bg-gray-50 outline-none text-sm text-gray-600" /></div>
        <div className="flex gap-3">
          <div className="flex-1"><label className="text-xs font-bold text-gray-500 block mb-1">付款人 (先墊錢的)</label><select value={payerId} onChange={e=>setPayerId(e.target.value)} className="w-full border border-gray-200 p-2 rounded-lg bg-gray-50 text-[#52B4CC] font-bold outline-none">{members.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></div>
      ...
      {splitMode === 'equal' && (
        <div className="bg-white p-4 rounded-xl shadow-sm space-y-4 border-l-4 border-[#52B4CC]">
          ...
          <div>
            ...
            {totalAmount > 0 && selectedSplitters.length > 0 && ( <div className="mt-4 p-3 bg-[#FCFCFC] border border-[#C5E6EE] rounded-lg text-sm text-[#3B93A8] flex justify-between"><span className="font-medium">每人約負擔：</span><span className="font-bold text-lg">{formatMoney(totalAmount/selectedSplitters.length, (totalAmount/selectedSplitters.length) % 1 !== 0)}</span></div> )}
          </div>
        </div>
      )}
      {splitMode === 'unequal' && (
      ...
      {initialData ? (
        <div className="flex gap-3 pt-2">
          ...
          <button onClick={handleSave} className="flex-1 bg-[#52B4CC] text-white p-4 rounded-xl font-bold text-lg shadow-lg">儲存修改</button>
        </div>
      ) : (
...
function ExpenseList({ project, expenses, members, onEdit }) {
  ...
  const executeDelete = async (e, id) => { e.stopPropagation(); try { await deleteDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'expenses', id)); setDeletingExpenseId(null); } catch(e){} };
  const filtered = expenses.filter(exp => filterMemberId === 'all' || exp.payerId === filterMemberId || Object.keys(exp.splits).includes(filterMemberId));
  const grouped = filtered.reduce((acc, exp) => { if (!acc[exp.date]) acc[exp.date] = []; acc[exp.date].push(exp); return acc; }, {});

  if (expenses.length === 0) return <div className="flex flex-col items-center justify-center p-12 text-center text-[#8BCDDD]"><Receipt size={64} className="opacity-40 mb-4"/><p className="font-bold text-gray-500">還沒有花費紀錄</p></div>;
                    ...
                    <div className="bg-gray-50 p-4 border-t border-gray-100 text-sm">
                      ...
                      <div className="flex justify-end pt-3 border-t border-gray-200 mt-2">
                        {deletingExpenseId === exp.id ? (
                           <div className="flex w-full justify-between items-center bg-red-50 p-2 rounded-lg border border-red-100"><span className="text-red-600 text-sm font-bold">確定刪除？</span><div className="flex gap-2"><button onClick={(e) => { e.stopPropagation(); setDeletingExpenseId(null); }} className="px-3 py-1.5 bg-white rounded-lg text-sm font-bold border border-gray-200">取消</button><button onClick={(e) => executeDelete(e, exp.id)} className="px-3 py-1.5 bg-red-500 text-white rounded-lg text-sm font-bold">刪除</button></div></div>
                        ) : (
                          <div className="flex gap-2"><button onClick={(e) => { e.stopPropagation(); onEdit(exp); }} className="flex items-center gap-1 px-3 py-1.5 bg-white border border-[#C5E6EE] text-[#52B4CC] rounded-lg font-bold"><Edit2 size={16}/> 編輯</button><button onClick={(e) => { e.stopPropagation(); setDeletingExpenseId(exp.id); }} className="flex items-center gap-1 px-3 py-1.5 bg-white border border-red-100 text-red-500 rounded-lg font-bold"><Trash2 size={16}/> 刪除</button></div>
                        )}
                      </div>
                    </div>
                  )}
...
function SettlementView({ project, expenses, members }) {
  ...
  const usedDatesForCurrencies = useMemo(() => {
    const datesMap = {}; expenses.forEach(e => { if (e.currency !== 'TWD') { if (!datesMap[e.date]) datesMap[e.date] = new Set(); datesMap[e.date].add(e.currency); } });
    return Object.fromEntries(Object.entries(datesMap).map(([d, s]) => [d, Array.from(s)]));
  }, [expenses]);

  const handleRateChange = async (key, value) => { const newRates = { ...rates, [key]: Number(value) }; setRates(newRates); try { await updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'projects', project.id), { rates: newRates }); } catch(e){} };
  const handleModeChange = async (mode) => { setRatesMode(mode); try { await updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'projects', project.id), { ratesMode: mode }); } catch(e){} };

  const settlementData = useMemo(() => {
    const exactBalances = {}; const exactPersonalTotals = {}; members.forEach(m => { exactBalances[m.id] = 0; exactPersonalTotals[m.id] = 0; });
    expenses.forEach(exp => {
      let rate = 1; if (exp.currency !== 'TWD') rate = ratesMode === 'unified' ? (rates[exp.currency] || 1) : (rates[`${exp.date}_${exp.currency}`] || rates[exp.currency] || 1);
      const totalTWD = exp.totalAmount * rate; if (exactBalances[exp.payerId] !== undefined) exactBalances[exp.payerId] += totalTWD;
      Object.entries(exp.splits).forEach(([uid, amt]) => { if (exactBalances[uid] !== undefined) { const amtTWD = amt * rate; exactBalances[uid] -= amtTWD; exactPersonalTotals[uid] += amtTWD; } });
    });
    const balances = {}; const personalTotals = {}; let sum = 0; let maxAbsId = null; let maxAbs = -1;
    members.forEach(m => {
      const roundedBalance = Math.round(exactBalances[m.id] || 0); balances[m.id] = roundedBalance; personalTotals[m.id] = Math.round(exactPersonalTotals[m.id] || 0); sum += roundedBalance;
      if (Math.abs(roundedBalance) > maxAbs) { maxAbs = Math.abs(roundedBalance); maxAbsId = m.id; }
    });
    if (sum !== 0 && maxAbsId) balances[maxAbsId] -= sum;
    const debtors = []; const creditors = []; Object.entries(balances).forEach(([id, amt]) => { if (amt < 0) debtors.push({ id, amount: Math.abs(amt) }); else if (amt > 0) creditors.push({ id, amount: amt }); });
    debtors.sort((a,b) => b.amount - a.amount); creditors.sort((a,b) => b.amount - a.amount);
    const transfers = []; let d = 0, c = 0;
    while (d < debtors.length && c < creditors.length) {
      const minAmount = Math.min(debtors[d].amount, creditors[c].amount); if (minAmount > 0) transfers.push({ from: debtors[d].id, to: creditors[c].id, amount: minAmount });
      debtors[d].amount -= minAmount; creditors[c].amount -= minAmount; if (debtors[d].amount === 0) d++; if (creditors[c].amount === 0) c++;
    }
  ...
  const exportCSV = async () => {
    ...
    expenses.sort((a,b)=>new Date(a.date)-new Date(b.date)).forEach(exp => {
      ...
      let rate = 1; if (exp.currency !== 'TWD') rate = ratesMode === 'unified' ? (rates[exp.currency] || 1) : (rates[`${exp.date}_${exp.currency}`] || 1);
